...
export default function LoginPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signInWithOtp({
        email,
        options: {
            shouldCreateUser: true // Allow sign up via login page too
        }
    })
        ...
        <div className="w-full max-w-md space-y-8 bg-background p-8 rounded-2xl border border-border shadow-lg">
            <div className="text-center space-y-2">
                ...
                <p className="text-muted-foreground">
                    {step === 'email' ? 'Enter your email to sign in or sign up' : `We sent a code to ${email}`}
                </p>
            </div>
            ...
            <Button variant="outline" type="button" className="w-full h-11" onClick={async () => {
