      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        >
            ...
            <div className="flex items-center gap-2">
                <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 text-[10px] font-black uppercase tracking-widest animate-pulse">
                    ● System Online
                </Badge>
                <p className="text-muted-foreground text-xs font-medium">Intelligence feed active and synced.</p>
            </div>
        </motion.div>
        ...
        >
             <Button variant="outline" size="sm" className="hidden sm:flex rounded-full border-2 font-bold uppercase tracking-widest text-[10px] h-10 px-6" asChild>
                 <Link href="/admin/orders">Order History</Link>
             </Button>
             <Button size="sm" className="rounded-full shadow-xl shadow-primary/20 font-bold uppercase tracking-widest text-[10px] h-10 px-6">
                 Export Report
             </Button>
        </motion.div>
      </div>
        ...
        {[
            ...
            >
                <Card className="border-2 hover:border-primary/20 transition-all duration-500 group relative overflow-hidden h-full">
                    ...
                    <CardContent>
                        <div className="text-2xl md:text-3xl font-black italic tracking-tighter">{item.value}</div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        ...
        <Card className="col-span-full lg:col-span-4 border-2 overflow-hidden bg-white/50 backdrop-blur-xl">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
                <CardTitle className="text-lg font-black uppercase tracking-tighter italic">Growth Dynamics</CardTitle>
                <CardDescription>Performance trends for the current period.</CardDescription>
            </div>
            <div className="flex gap-2">
                <Badge variant="secondary" className="rounded-full bg-zinc-100 text-zinc-600 font-bold uppercase text-[9px]">2025</Badge>
            </div>
          </CardHeader>
        ...
        <Card className="col-span-full lg:col-span-3 border-2 bg-zinc-900 text-white overflow-hidden flex flex-col">
          <CardHeader className="flex flex-row items-center justify-between border-b border-white/10 pb-4">
            <div>
                ...
                <CardDescription className="text-zinc-500">Real-time ecosystem events.</CardDescription>
            </div>
                    ...
                    activity.map((item: any, i: number) => (
                        ...
                        >
                            <div className={cn(
                                ...
                                item.type === 'order' && "bg-primary/20 border-primary/30 text-primary",
                                item.type === 'review' && "bg-amber-500/20 border-amber-500/30 text-amber-500",
                                item.type === 'newsletter' && "bg-blue-500/20 border-blue-500/30 text-blue-500"
                            )}>
                            ...
                            <div className="flex-1 min-w-0 border-b border-white/5 pb-4">
                                <p className="text-sm font-black italic uppercase leading-none truncate group-hover:text-primary transition-colors">{item.title}</p>
                                <p className="text-xs text-zinc-500 truncate font-medium mt-1.5">{item.description}</p>
                                <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest mt-1.5 flex items-center gap-2">
                                    <span className="h-1 w-1 rounded-full bg-zinc-700" />
                                    {mounted ? formatDistanceToNow(new Date(item.time), { addSuffix: true }) : 'Just now'}
                                </p>
                            </div>
                        </motion.div>
                    ))
          ...
          <div className="p-4 bg-white/5 border-t border-white/10">
              <Button variant="ghost" className="w-full text-zinc-400 hover:text-white hover:bg-white/10 text-[10px] font-black uppercase tracking-[0.2em] h-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        ...
        <Card className="col-span-full lg:col-span-4 border-2 overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between border-b bg-zinc-50/50">
                <div>
                    <CardTitle className="text-lg font-black uppercase tracking-tighter italic">Recent Transmissions</CardTitle>
                    <CardDescription>Latest order signals received.</CardDescription>
                </div>
                <Button asChild size="sm" variant="outline" className="font-bold border-2 rounded-full uppercase tracking-widest text-[10px] h-8 px-4 pr-3">
                ...
                <Table>
                    <TableHeader className="bg-muted/10">
                        <TableRow className="border-b-0 hover:bg-transparent">
                            <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] px-6 h-12 text-muted-foreground">Order</TableHead>
                            <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] h-12 text-muted-foreground text-center">Identity</TableHead>
                            <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] h-12 text-muted-foreground text-center">Status</TableHead>
                            <TableHead className="text-right text-[10px] font-black uppercase tracking-[0.2em] pr-6 h-12 text-muted-foreground">Total</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {recentOrders.map((order) => (
                            <TableRow key={order.id} className="hover:bg-zinc-50 transition-colors border-b group">
                                ...
                                <TableCell className="text-center py-4">
                                    <span className="text-xs font-black uppercase italic tracking-tight">{order.profiles?.name || order.shipping_name || 'Anonymous'}</span>
                                </TableCell>
                                <TableCell className="text-center py-4">
                                    <Badge variant="outline" className={cn(
                                        ...
                                        order.status === 'delivered' && "bg-zinc-50 text-zinc-600 border-zinc-100"
                                    )}>
                                        {order.status}
                                    </Badge>
                                </TableCell>
                                <TableCell className="text-right font-black italic pr-6 text-sm py-4">₹{order.total.toLocaleString()}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
        ...
        <div className="col-span-full lg:col-span-3 space-y-6">
            <Card className="border-2 overflow-hidden">
                <CardHeader className="pb-2">
                    <CardTitle className="text-lg font-black uppercase tracking-tighter italic">Hot Inventory</CardTitle>
                    <CardDescription>Top performing drops.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    {topProducts.length === 0 ? (
                        <p className="text-center text-muted-foreground py-8 text-xs font-bold uppercase italic tracking-widest">Awaiting Data...</p>
                    ) : (
                            ...
                            <div key={product.id} className="flex items-center gap-3 group">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                        <p className="text-xs font-black uppercase truncate italic group-hover:text-primary transition-colors">{product.name}</p>
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        ...
                                        <div className="flex-1 h-1 bg-zinc-100 rounded-full overflow-hidden">
                                            <div 
                                                ...
                                                style={{ width: `${Math.min((product.sale_count / (topProducts[0]?.sale_count || 1)) * 100, 100)}%` }} 
                                            />
                                        </div>
                                    </div>
                                </div>
            ...
            <Card className="border-2 overflow-hidden bg-zinc-50/50">
                <CardHeader className="pb-2">
                    ...
                    <CardDescription>Category performance distribution.</CardDescription>
                </CardHeader>
