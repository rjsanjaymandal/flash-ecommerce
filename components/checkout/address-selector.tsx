...
import { getAddresses, addAddress, Address } from '@/lib/services/address-service'
                 ...
                 {addresses.map(addr => (
                     <div 
                        ...
                        className={cn(
                            ...
                            selectedId === addr.id ? "border-primary bg-primary/5 shadow-sm" : "border-border bg-card"
                        )}
                     >
                        {selectedId === addr.id && (
                        ...
                        {addr.is_default && <span className="text-[10px] bg-secondary text-secondary-foreground px-1.5 py-0.5 rounded-full mt-2 inline-block">Default</span>}
                     </div>
                 ))}

                 <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                     <DialogTrigger asChild>
                         <Button variant="outline" className="h-[100px] w-full border-dashed flex flex-col gap-2 hover:bg-muted/50 rounded-xl">
                         ...
                         <form action={handleAddAddress} className="space-y-4 pt-4">
                             <div className="grid grid-cols-2 gap-4">
                                 ...
                                 <div className="space-y-2 col-span-2">
                                     ...
                                     <Input id="address1" name="address1" placeholder="Street, Sector" required />
                                 </div>
                                 <div className="space-y-2 col-span-2">
                                     <Label htmlFor="address2">Address Line 2 (Optional)</Label>
                                     <Input id="address2" name="address2" placeholder="Appartment, Landmark" />
                                 </div>
