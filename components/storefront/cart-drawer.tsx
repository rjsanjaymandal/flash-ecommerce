        ...
        <div className="p-5 border-b border-border/50 bg-background/50 backdrop-blur-md z-10">
            <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-black uppercase tracking-tight flex items-center gap-2 italic">
                    ...
                    Your Bag <span className="text-sm not-italic font-medium text-muted-foreground bg-muted px-2 py-0.5 rounded-full">({items.length})</span>
                </h2>
            ...
            {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground space-y-6 text-center p-8">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-sm max-w-xs mx-auto">Looks like you haven't added any gear to your loadout yet.</p>
                    </div>
                    <Button onClick={() => setIsCartOpen(false)} size="lg" className="rounded-full px-8 font-bold">
                        Start Shopping
                    </Button>
                </div>
            ) : (
                                ...
                                <div className="h-28 w-24 bg-muted rounded-xl overflow-hidden shrink-0 border border-border/50 relative">
                                    ...
                                    {item.maxQuantity === 0 && (
                                        <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-[2px]">
                                            <span className="text-[10px] uppercase font-bold text-white bg-destructive px-2 py-1 rounded-full">Sold Out</span>
                                        </div>
                                    )}
                                </div>
                                <div className="flex-1 flex flex-col justify-between py-1">
                                    <div className="space-y-1">
                                        <div className="flex justify-between items-start gap-2">
                                            <h3 className={cn("font-bold text-sm uppercase tracking-wide line-clamp-2 leading-tight", item.maxQuantity === 0 && "text-muted-foreground line-through decoration-destructive")}>{item.name}</h3>
                                            <button 
                                        ...
                                        <p className="text-xs text-muted-foreground font-medium">{item.size} / {item.color}</p>
                                        {item.maxQuantity === 0 && (
                                            <p className="text-[10px] text-destructive font-bold flex items-center gap-1">
                                                <AlertCircle className="h-3 w-3" /> Item is out of stock
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between mt-2">
                                        ...
                                        <span className={cn("font-mono text-sm font-bold", item.maxQuantity === 0 && "text-muted-foreground line-through")}>{formatCurrency(item.price * item.quantity)}</span>
                                    </div>
                                </div>
                            </motion.div>
                        ))}
                ...
                <Button 
                    ...
                    onClick={(e) => {
                        if (hasOutOfStockItems) {
                            e.preventDefault()
                            const oosItems = items.filter(i => i.maxQuantity === 0 || i.quantity > i.maxQuantity)
                            oosItems.forEach(i => removeItem(i.productId, i.size, i.color))
