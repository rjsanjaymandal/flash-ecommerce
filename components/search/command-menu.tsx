      ...
      <CommandList className="scrollbar-hide">
        ...
        <CommandGroup heading={<span className="text-xs font-black uppercase tracking-widest text-accent ml-1">Trending Collections</span>}>
              <div className="grid grid-cols-2 gap-2 p-2 focus-within:ring-0">
                  <button 
                    onClick={() => runCommand(() => router.push('/shop'))}
        ...
        <CommandGroup heading={<span className="text-xs font-black uppercase tracking-widest text-primary ml-1">Latest Products</span>}>
            {items.slice(0, 8).map((item) => (
                ...
                >
                    <div className="relative h-12 w-12 rounded-xl overflow-hidden border border-border group-hover:border-primary transition-colors mr-3">
                        <img src={item.main_image_url} className="h-full w-full object-cover group-hover:scale-110 transition-transform duration-500" alt={item.name} />
                    </div>
                    <div className="flex flex-col flex-1 gap-1">
                        <span className="font-black uppercase tracking-tight leading-none group-hover:text-primary transition-colors">{item.name}</span>
                        {item.price && <span className="text-xs font-bold text-muted-foreground">{formatCurrency(item.price)}</span>}
                    </div>
                    <ArrowRight className="h-4 w-4 opacity-0 -translate-x-2 transition-all group-aria-selected:opacity-100 group-aria-selected:translate-x-0 group-hover:opacity-100 group-hover:translate-x-0" />
                </CommandItem>
            ))}
        </CommandGroup>
